    ...
    ***************************************************************************** */
    ...
    var Field = /** @class */ (function() {
        ...
        Field.prototype.buildGrid = function() {
            ...
            if (chooseVector === 'wind' || isBoth) {
                if (iscurrent) {

                } else {
                    /**********wind*********/
                    /* for (var j = 0; j < rows; j++) {
                        grid[j] = grid[j].slice(180).concat(grid[j].slice(0, 180))
                    } */
                    grid.reverse()
                }
            } else {
                ...
                for (let k = 0; k < 10; k++) {
                    ...
                    for (var i = 0; i < cols; i++) {
                        row[i] = (new Vector(0, 0));
                    }
                    row.push(row[0]);
                    grid.push(row);
                }
                /* for (var j = 0; j < rows; j++) {
                    grid[j] = grid[j].slice(150).concat(grid[j].slice(0, 150))
                }
                let temp = 0,
                    isAdd = true
                for (let i = 20; i < rows - 20; i++) {
                    grid[i] = grid[i].slice(360 - temp).concat(grid[i].slice(0, 360 - temp))
                    temp += 1
                } */
            }
            return grid;
    ...
    class CesiumWind {
        constructor(data, options = {}, vHeight) {
            this.canvas = null;
            this.wind = null;
            this.field = null;
            this.viewer = null;
        ...
        render(canvas) {
            ...
            if (canvas && !this.wind) {
                ...
                if (ctx) {
                    ...
                    if (this.vHeight !== 0) {
                        this.wind.project = this.project1000.bind(this);
                    } else {
